...
export class DbHelperBNRE {
  ...
  readonly assignMissingSettlement = async () => {
    ...
    const entries = await this.prisma.koboAnswers.findMany({where: {uuid: {in: Enum.keys(mapUuiSettlement)}}}).then(_ => _.map(DbKoboFormHelper.setTagPropertyTsType<Bn_Re.T>()))
